...
(function () {
    ...
    const dbName = 'Elo';
    const localEditsStore = "localedits";
    const serverDataStore = "serverdata";
    const keyPath = 'picId';
    const readWrite = 'readwrite';
    const indexName = 'lastUpdated';
    const prevDirection = 'prev';

    const db = idb.openDB(dbName, 1, {
        upgrade(db) {
            db.createObjectStore(localEditsStore, { keyPath: keyPath }).createIndex(indexName, indexName);
            db.createObjectStore(serverDataStore, { keyPath: keyPath });
        },
    });
    ...
    window.localStore = {
        ...
        getFirstFromIndex: async (storeName) => {
            const cursor = await (await db).transaction(storeName).store.index(indexName).openCursor(null, prevDirection);
